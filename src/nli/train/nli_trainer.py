...
def get_features(data,qrels,corpus,key='snippet',k=3): # creates input [Claim][Questions][Evidences]
  features = []
  for index, fact in tqdm.tqdm(enumerate(data),total=len(data)):
    ...
    if(str(index) in qrels):
        evidences = [corpus[str(idx)]['title'] + ' ' + corpus[str(idx)][key] for idx in list(qrels[str(index)].keys())[:k]]

    feature = "[Claim]:"+claim+" [Evidences]: "+"[SEP]".join(evidences)
    print(feature)
...
for k in [1,5,7,10]:
            with open(f"train_ret_out.json") as fp:
                train_qrels = json.load(fp)

            with open(f"val_ret_out.json") as fp:
            ...
            output_dir = "output_nli_training"
            ...
            train_features = get_features(train_dataset,train_qrels,corpus,k=k)

            #Loading train features
            val_features = get_features(val_dataset,val_qrels,corpus,k=k)
            ...
            class EarlyStopping:
                ...
                def __init__(self, patience=3, verbose=False, delta=0.0, path='checkpoint.pt', trace_func=print):
                    """
                    Args:
                ...
                def __call__(self, val_loss, model):
                    ...
                    score = -val_loss
                    print("Here",score,self.best_score)
