...
def get_verification(config):
    ...
    for index, fact in tqdm.tqdm(enumerate(facts),total=len(facts)):        
        ...
        if(str(index) in qrels):
            top_k_docs = [corpus[idx]['title']+' '+corpus[idx]['snippet'] for idx in qrels[str(index)].keys()]
        ...
        if len(top_k_docs) > 0:
            ...
            pred_label,confidence = nli_model.predict("[Claim]:"+claim)
            input = ""
...
for k in [10]:
    for option in options:
        ...
        parser.add_argument("--qrels_path", type=str,
                            default=f"dataset_experiments/snippets/retrieval/out/final_setup_temporal/test/claim_only_ret.json",
                            help="Path to the corpus")
        parser.add_argument("--base_model", type=str,
                            default="FacebookAI/roberta-large-mnli", help="Path to the base model")
        parser.add_argument("--model_path", type=str,
                            default="dataset_experiments/snippets/nli/out/train/claim_no_evidence_final/model_weights",
                            help="Path to the model")
        # parser.add_argument("--model_path", type=str,
        #                     default="QuantCheck/model/model_roberta_large_claimdecomp_final/model_weights.zip",
        #                     help="Path to the model")
        parser.add_argument("--output_path", type=str,
                            default=f"dataset_experiments/snippets/nli/out/inference/claim_no_evidence_final",
